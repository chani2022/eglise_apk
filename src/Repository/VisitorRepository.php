...
class VisitorRepository extends ServiceEntityRepository
{
    ...
    public function getVisitor(array $dates): array
    {
        return $this->createQueryBuilder('v')
            ->select(['COUNT(v.id) as nb_visitor', 'v.visited_at'])
            ->where('v.visited_at BETWEEN :deb AND :df')
            ->setParameter("deb", $dates[0])
            ->setParameter("df", $dates[1])
            ->groupBy("v.visited_at")
            ->getQuery()
            ->getArrayResult();
    }
